...
//    //书本路径
    ...
    private PageFactory(Context context) {
        ...
        mPaint.setTypeface(typeface);//字体类型
    ...
    private void measureMarginWidth(){
        float wordWidth =mPaint.measureText("\u3000");  // \u3000  中文(全角)空格
    ...
    public void onDraw(Bitmap bitmap,List<String> m_lines,Boolean updateCharter) {
        ...
        if (currentPage != null && bookList != null){
            new Thread() {
                ...
                public void run() {
                    super.run();
                    values.put("begin",currentPage.getBegin());//将开始的位置存储
...
//                m_mbBufLen = mBookUtil.getBookLen();
    ...
    public List<String> getNextLines(){//获取下一行数
...
//                    height +=  paragraphSpace;
    ...
    public void currentPage(Boolean updateChapter){
        onDraw(mBookPageWidget.getCurPage(),currentPage.getLines(),updateChapter);//绘制当前页面
